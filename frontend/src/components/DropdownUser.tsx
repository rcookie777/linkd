...
// import getNameFromUser from '../common/AWS';
import { AuthContext } from '../App';

const DropdownUser = () => {
  const [dropdownOpen, setDropdownOpen] = useState(false);

  const {access_token, setAccessToken, user_data, setUserData} = useContext(AuthContext);
  const navigate = useNavigate();
  
  const trigger = useRef<any>(null);
  const dropdown = useRef<any>(null);
  // const isLoggedin = useUserStatus();
  // const name = getNameFromUser();
    ...
    <div className="relative">
      ...
      >
        {!access_token ? (
          ...
          <span className="hidden text-right lg:block">
                     {user_data ? ( <div>{user_data.login} </div> ) : ( <span></span> )}
            </span>
        )}


        <span className="h-12 w-12 rounded-full">
          <img src={user_data.avatar_url} alt="User" />
        </span>
